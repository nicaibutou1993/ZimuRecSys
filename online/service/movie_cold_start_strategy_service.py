...
"""
def get_movie_cold_start_rec(user_id):

    rec_cold_match = ""
    user_profile_label = get_user_profile(user_id)

    if user_profile_label:

        cold_movies = client.hmget(REDIS_COLD_MOVIES, user_profile_label)[0]
...
"""
def update_cold_movies_list(movie_id):

    movie_info = client.hmget(REDIS_MOVIE_INFO, movie_id)[0]

    if movie_info:
        label = GENRE2LABELMAP.get(movie_info[1])
        cold_movies = client.hmget(REDIS_COLD_MOVIES, label)[0]
        ...
        if cold_movies:
            ...
            if click_num >= cold_movie_need_display_num:
                cold_movies.pop(movie_id)
            else:
                cold_movies[movie_id] = click_num

            client.hmset(REDIS_COLD_MOVIES,{label:json.dumps(cold_movies)})
...
def get_user_profile(user_id):
    ...
    profile_10 = client.hmget(REDIS_SHORT_TERM_INTEREST_10, user_id)[0]

    profile_20 = client.hmget(REDIS_MIDDLE_TERM_INTEREST_20, user_id)[0]

    profile_50 = client.hmget(REDIS_LONG_TERM_INTEREST_50, user_id)[0]

    user_profile_label = ""
    if None != profile_10 and None != profile_20 and None != profile_50:
        profile_10 = Counter(json.loads(profile_10))
        profile_20 = Counter(json.loads(profile_10))
        profile_50 = Counter(json.loads(profile_10))
