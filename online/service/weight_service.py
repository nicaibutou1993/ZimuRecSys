...
class WeightService(object):
    ...
    def get_user_weight_data(self, user_id):
        ...
        rec_num = self.rec_genre_movie_num(weights)

        norm_weights = self.__norm_weight(weights)

        return norm_weights, rec_num

    """更新用户权重值"""

    def update_user_weight_data(self, user_id, click_id, genre_movie_nums):
        self.update_weight(user_id, click_id, genre_movie_nums)

    """ 这里归一化权重，这里加了 0.5 主要是为了和电影自身的评分，
        进行加和平均 ，否则 每一个类别的权重都太低了
    """

    def __norm_weight(self, weights):
    ...
    def update_weight(self, user_id, id, genre_movie_nums, last_weight=None, is_update_redis=True, is_print=True):
            ...
            for label, num in genre_movie_nums.items():
                if label != id:
                    ...
                    if current_weight <= 1:
                        ...
                        """
                        if current_weight <= self.basic_score:
        ...
        if is_print:
            ...
            print("推荐后的数量： ", sorted(self.rec_genre_movie_num(weight).items(), key=lambda x: x[0]))
            self.__print_weight(weight)
    ...
    def __print_weight(self, weights):
    ...
    def rec_genre_movie_num(self, weights):
        total = float(sum(weights.values()))
        ...
        for label, weight in weights.items():
            num = math.floor(weight / total * self.rec_num)

            if round(num / self.rec_num, 2) > 0.4:
                num = int(0.4 * self.rec_num)

            _total_num += num

            genre_movie_num[label] = num

        left_num = self.rec_num - _total_num
        if left_num > 0:
